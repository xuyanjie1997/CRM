...
init();
function init(){
	ajax('/info/selectDel','','json',function(data){
		console.log(data)
		var html = "";
		$.each(data.list,function(i,dom){
			...
			var email = dom.email;
			var d = {code:code,name:name,cby:cby,ctime:ctime,email:email}
...
function back(code){
	ajax("/info/back" , {code:code} , "text" , function(data){
		if(data == 1){
			layer.msg("数据还原成功！")
			init();
		}else{
...
function delCompletely(code){
	openConfirm(function(index){
		ajax("/info/delCompletely" , {code:code} , "text" , function(data){
			if(data == 1){
				layer.msg("数据已经彻底删除！")
				init();
			}else{
